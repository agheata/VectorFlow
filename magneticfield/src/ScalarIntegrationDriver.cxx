...
// }
    ...
    htemp = fSafetyFactor * h * vectorflow::Math::Pow(errmax_sq, 0.5 * fPowerShrink);
...
// }
  ...
  if (errmax_sq > fErrcon * fErrcon) {
    hnext = GetSafety() * h * vectorflow::Math::Pow(errmax_sq, 0.5 * GetPowerGrow());
  } else {
                                                   ...
                                                   double hstepCurrent) // current step size
{
  ...
  if (errMaxNorm > 1.0) {
    // Step failed; compute the size of retrial Step.
    hnew = GetSafety() * hstepCurrent * vectorflow::Math::Pow(errMaxNorm, GetPowerShrink());
  } else if (errMaxNorm > 0.0) {
    // Compute size of next Step for a successful step
    hnew = GetSafety() * hstepCurrent * vectorflow::Math::Pow(errMaxNorm, GetPowerGrow());
  } else {
