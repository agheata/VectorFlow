...
int main(int argc, char *args[])
{
  ...
  const double hminimum  = 0.0001 * geant::units::millimeter; // Minimum step = 0.1 microns
  ...
  // = geant::units::GeV / Constants::c_light;     // OLD
...
//   This builds on its previous step to create the solution !
    ...
    if (j > 0) // Step 0 did not move -- printed the starting values
    {
      ...
      cout.setf(ios_base::fixed);
      cout.precision(2);
      cout << setw(wd) << atan2(yout[1], yout[0]) / geant::units::degree;
      // atan(yout[1]/yout[0])/degree;
      if (printRef) cout << setw(wd) << atan2(youtX[1], youtX[0]) / geant::units::degree;
